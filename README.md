...
|---|---------|-------|-----------|------------|
| 1 | neural-network-from-scratch | Undergraduate | NumPy only | Backpropagation, autodiff-free layers, gradient checking, optimizers (SGD/Momentum/Adam/AdamW), BatchNorm, Dropout |
| 2 | cnn-image-classification | Undergraduate / Master | PyTorch | ResNet, data augmentation (Cutout), cosine LR schedule, label smoothing, mixed precision |
| 3 | mini-gpt | Master | PyTorch | Transformer decoder, causal self-attention, pre-LN blocks, AdamW + warmup/cosine, top-k sampling |
| 4 | ddpm-mnist | PhD | PyTorch | Denoising Diffusion Probabilistic Models, U-Net with time conditioning, DDPM & DDIM samplers, EMA |
| 5 | dqn-reinforcement-learning | Master / PhD | PyTorch + Gymnasium | Deep Q-Networks, replay buffer, target networks, Double DQN, dueling architecture, epsilon-greedy |
